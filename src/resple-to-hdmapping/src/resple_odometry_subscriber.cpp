    ...
    while (bag.has_next())
    {
        ...
        if (msg->topic_name == "/current_scan") {
            ...
            if (!cloud_msg || cloud_msg->data.empty()) {
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("RESPLEFrame"), "Empty PointCloud2 message on /current_scan");
                continue;
            }
    
            size_t num_points = cloud_msg->width * cloud_msg->height;
    
            RCLCPP_INFO(rclcpp::get_logger("RESPLEFrame"), "Processing %zu points from /current_scan", num_points);
            ...
            RCLCPP_INFO(rclcpp::get_logger("RESPLEFrame"), "Added %zu points (total: %zu)", num_points, points_global.size());
        }
